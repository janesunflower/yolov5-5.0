...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        if '*' in p:
            ...
            raise Exception('ERROR: {} does not exist'.format(f))
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            # Read video
            self.mode = 'video'
            ret_val, img0 = self.cap.read() # 544,960,3
            if not ret_val:
                self.count += 1
                self.cap.release()
        ...
        # Padded resize
        img = letterbox(img0, self.img_size, stride=self.stride, auto=self.auto)[0] # 384, 640
